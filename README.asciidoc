...
Text substitution is primitive, but I was unable to think of another way to
achieve an Awk-esque feel. Purists unable to tolerate the `NN_FUN` macro will
need more code:
...
need a throwaway defnition of yySymType.
...
Pike then demonstrates how nesting structural expressions lead to a few
simple text editor commands that print all lines containing "rob" but not
"robot". Though Nex fails to separate looping from matching, a corresponding
program is bearable:
...
Unlike Bison, we must import `fmt` even if we don't use it, since code
generated by goyacc needs it. Also, the `%union` is mandatory; it generates
`yySymType`.
...
For brevity we're working in the `main` package. In a larger project one
might want to export a function that calls `yyParse()`. This is fine,
provided the parser and the lexer are both in the same package.

However, this ties together parsers and lexers. We cannot use multiple parsers
with the same lexer, nor vice versa. Though irrelevant for typical applications,
this is somewhat dissatisfying because the limitation is artificial.

One workaround is to search and replace the first letters of `yyParse`
and `yySymType` with uppercase letters. Perhaps there ought to be a goyacc
command-line option to arrange this.
